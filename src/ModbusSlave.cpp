/*
    ModbusSlave.cpp - Source for ModbusSlave Base Library
...
sTRegister* ModbusSlave::searchRegister(word address) {
    sTRegister *reg = _regs_head;
...
void ModbusSlave::addReg(word address, word value) {
    sTRegister *newreg;

  newreg = (sTRegister *) malloc(sizeof(sTRegister));
...
bool ModbusSlave::Reg(word address, word value) {
    sTRegister *reg;
...
word ModbusSlave::Reg(word address) {
    sTRegister *reg;
    reg = this->searchRegister(address);
    if(reg)
        return(reg->value);
...
void ModbusSlave::addHreg(word offset, word value) {
    this->addReg(offset + 40001, value);
}

bool ModbusSlave::Hreg(word offset, word value) {
    return Reg(offset + 40001, value);
}

word ModbusSlave::Hreg(word offset) {
    return Reg(offset + 40001);
}

#ifndef USE_HOLDING_REGISTERS_ONLY
    void ModbusSlave::addCoil(word offset, bool value) {
        this->addReg(offset + 1, value?0xFF00:0x0000);
    }

    void ModbusSlave::addIsts(word offset, bool value) {
        this->addReg(offset + 10001, value?0xFF00:0x0000);
    }

    void ModbusSlave::addIreg(word offset, word value) {
        this->addReg(offset + 30001, value);
    }

    bool ModbusSlave::Coil(word offset, bool value) {
        return Reg(offset + 1, value?0xFF00:0x0000);
    }

    bool ModbusSlave::Ists(word offset, bool value) {
        return Reg(offset + 10001, value?0xFF00:0x0000);
    }

    bool ModbusSlave::Ireg(word offset, word value) {
        return Reg(offset + 30001, value);
    }

    bool ModbusSlave::Coil(word offset) {
        if (Reg(offset + 1) == 0xFF00) {
            return true;
        } else return false;
    }

    bool ModbusSlave::Ists(word offset) {
        if (Reg(offset + 10001) == 0xFF00) {
            return true;
        } else return false;
    }

    word ModbusSlave::Ireg(word offset) {
...
#ifndef USE_HOLDING_REGISTERS_ONLY
void ModbusSlave::readCoils(word startreg, word numregs) {
    //Check value (numregs)
...
void ModbusSlave::readInputStatus(word startreg, word numregs) {
    //Check value (numregs)
    if (numregs < 0x0001 || numregs > 0x07D0) {
...
void ModbusSlave::readInputRegisters(word startreg, word numregs) {
    //Check value (numregs)
    if (numregs < 0x0001 || numregs > 0x007D) {
...
void ModbusSlave::writeSingleCoil(word reg, word status) {
    //Check value (status)
...
void ModbusSlave::writeMultipleCoils(byte* frame,word startreg, word numoutputs, byte bytecount) {
    //Check value
