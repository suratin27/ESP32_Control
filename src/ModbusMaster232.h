...
Commented function crc16 not supported by ESP8266 - PDAControl
...
class ModbusMaster232{
  ...
  /*--------------- Definition by suratin --------------------------*/
   #define MB_TIMEOUT     20       //ms
   #define MB_RETRY       3       //times
    unsigned long timeout;        //Timeout variable
    uint8_t tryCount;           //Retry before connect error  
    
    ModbusMaster232();
    ModbusMaster232(uint8_t);                         //Arg(port)
    ModbusMaster232(uint8_t, uint8_t);                //Arg(port,slave)
    //Arg(Serial,Timeout,Retry count)
    ModbusMaster232(uint8_t, unsigned long,uint8_t);  //Arg(port,timeout,retry count)
    
    void begine(unsigned long);
    void begin();
    ...
    */
    ...
    void setConfig(uint8_t,unsigned long,uint8_t);
  ...
  private:
    ...
    volatile uint8_t  _u8MBSlave;                                         ///< Modbus slave (1..255) initialized in constructor
    volatile uint16_t _u16BaudRate;                                       ///< baud rate (300..115200) initialized in begin()
    static const uint8_t ku8MaxBufferSize                = 64;   ///< size of response/transmit buffers    
    volatile uint16_t _u16ReadAddress;                                    ///< slave register from which to read
    volatile uint16_t _u16ReadQty;                                        ///< quantity of words to read
    volatile uint16_t _u16ResponseBuffer[ku8MaxBufferSize];     ///< buffer to store Modbus slave response; read via GetResponseBuffer()
    volatile uint16_t _u16WriteAddress;                                   ///< slave register to which to write
    volatile uint16_t _u16WriteQty;                                       ///< quantity of words to write
    volatile uint16_t _u16TransmitBuffer[ku8MaxBufferSize];               ///< buffer containing data to transmit to Modbus slave; set via SetTransmitBuffer()
